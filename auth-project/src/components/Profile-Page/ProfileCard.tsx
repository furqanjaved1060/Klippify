...
const ProfileCard = ({buttons}) => {

    const {username} = useParams();

    const userData = useAuthUser(state => state.authUser.userData);

    let name = userData.fullName?.split(" ").map((curName) => curName.charAt(0).toUpperCase() + curName.slice(1));
    name = name?.join(" ");
       
    return (
    <div className="bg-[#007EFF1A] p-3 space-y-3 rounded-t-lg">
            ...
            <div className='xxs:min-w-85 flex flex-col xxs:flex-row items-center justify-between gap-5'>
                ...
                <div className='flex items-center gap-3'>
                    ...
                    <div className='relative size-17'>
                        ...
                        {buttons && <button className='cursor-pointer'><img src={edit} alt="" className='absolute size-5 bottom-0 right-0' /></button>}
                    </div>
