...
const BrPrPgPastCampaignsListItem: React.FC<Props> = ({curCampaign}) => {

    const {brandLogo, campaignName, campaignDesc, performanceAnalytics, creators} = curCampaign;

    const {avatarArray, rating} = creators;

    return (
    <li className="w-full xs:w-60 p-3 border-1 border-[#D9D9D9] rounded-lg space-y-2">

        <div className="flex items-center gap-2">
            <img src={brandLogo} alt="" className="size-8.5"/>
            <div className="space-y-1">
                <h3 className="text-xxs font-medium">{campaignName}</h3>
                <p className="text-[9px] font-normal text-[#33333380]">{campaignDesc}</p>
            </div>
        </div>

        <div className="space-y-1.5">
            <h4 className="text-xxs font-normal">Performance Analytics:</h4>
            <ul className="space-y-1.5">

                {performanceAnalytics.map((curAnalytic, index) => {
                    const {title, value} = curAnalytic;
                    return (
                        <li key={index} className="text-xxs font-normal text-[#33333380] flex items-center justify-between">
                            {title}<span className="font-medium text-[#000C21]">{value}</span>
                        </li>
                    )
                })}
        ...
        <div className="space-y-1.5">
            <h4 className="text-xxs font-normal">Creators:</h4>
            <div className="flex items-center justify-between">
                <ul className="flex items-center gap-1">

                    {avatarArray.map((curAvatar, index) => {
                        return (
                            <li key={index}><img src={curAvatar} alt="" className="size-6"/></li>
                        )
                    })}
                </ul>
...
export default BrPrPgPastCampaignsListItem;
