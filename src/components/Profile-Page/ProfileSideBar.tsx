    ...
    <aside className={`w-42 h-screen py-2 px-4 bg-white lg:flex flex-col justify-between fixed z-100 ${isOpen ? 'flex' : 'hidden'}`}>
      ...
      <div className="space-y-2">
        ...
        <div className='py-4 flex items-center gap-2 border-y-[0.25px] border-[#e1dfdf80]'>
          <img src={profile} alt="" className='size-8' />
          <div className="flex flex-col">
            <p className='text-[8px] font-medium text-[#33333380]'>DIGITAL CREATOR</p>
            <Link to={`/${username}/profile`} className='text-xxs font-medium'>{userData.fullName?.split(" ").map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")}</Link>
          </div>
        </div>

        <nav className='flex flex-col'>
          <NavLink to='dashboard' className={({isActive}) => `p-3 text-xxs text-[#33333380] font-medium flex gap-2 rounded-lg ${isActive && 'bg-gradient-to-r from-[#333EFF] to-[#5BCAFF] text-white'}`}><DashBoardIcon />Dashboard</NavLink>
          <NavLink to='campaigns' className={({isActive}) => `p-3 text-xxs text-[#33333380] font-medium flex gap-2 rounded-lg ${isActive && 'bg-gradient-to-r from-[#333EFF] to-[#5BCAFF] text-white'}`}><img src={dashboardIcon} className='size-4' />Campaigns</NavLink>
        </nav>
      </div>
